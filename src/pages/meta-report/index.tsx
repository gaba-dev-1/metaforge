...
import { Trophy, Star, Medal, Users, Shield, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function MetaReport() {
  ...
  const [pageSize, setPageSize] = useState(50); // Reasonable number to load
  
  const { data, isLoading, error, handleRetry, highlights, pagination } = useTftData(page, pageSize);
  ...
  // Highlight types - reduced to use only the first 3 for better performance
      ...
      <div className="mb-10 mt-6 flex flex-col lg:flex-row gap-6">
        ...
        <div className="lg:w-4/6">
          <Card className="p-0 overflow-hidden h-full backdrop-filter backdrop-blur-md bg-brown/5 border border-gold/30">
            ...
            {/* Pagination for comps only */}
            {activeTab === 'comps' && pagination && (
              <div className="p-4 flex justify-center items-center space-x-2">
                <button 
                  onClick={() => setPage(p => Math.max(1, p - 1))} 
                  disabled={page === 1}
                  className={`p-2 rounded ${page === 1 ? 'text-cream/30' : 'text-cream hover:bg-gold/20'}`}
                >
                  <ChevronLeft size={16} />
                </button>
                ...
                <button 
                  ...
                  className={`p-2 rounded ${page >= (pagination.totalPages || 1) ? 'text-cream/30' : 'text-cream hover:bg-gold/20'}`}
                >
