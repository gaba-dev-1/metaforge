...
export default function Home() {
  ...
  // Fixed: Single useMemo with all trait data that doesn't change
  const processedData = useMemo(() => {
    const allTraitsData = { ...traitsJson.origins, ...traitsJson.classes };
    
    // Process units
    const units = Object.entries(unitsJson.units as Record<string, any>)
      .map(([id, unit]) => ({ id, name: unit.name, icon: unit.icon, cost: unit.cost }))
      .sort((a, b) => a.cost - b.cost || a.name.localeCompare(b.name));
      
    // Process traits
    const traits = Object.entries(allTraitsData)
      .map(([id, trait]) => ({
        id, 
        name: trait.name, 
        icon: trait.icon,
    ...
    // Process items  
    const items = Object.entries(itemsJson.items as Record<string, any>)
  ...
  }, []); // No dependencies as this only depends on imported data

  // Filter options
  const costFilters = useMemo(() => 
    Array.from(new Set(processedData.allUnits.map(unit => unit.cost)))
      .sort()
      .map(cost => ({ id: cost.toString(), name: `${cost} 🪙` }))
  , [processedData.allUnits]);
  
  const categoryFilters = useMemo(() => 
    Array.from(new Set(processedData.allItems.map(item => item.category)))
      .filter(Boolean)
  ...
  , [processedData.allTraits, filters.traits]);
