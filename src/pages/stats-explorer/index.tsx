...
export default function StatsExplorer() {
  ...
  const filterOpts = useMemo(() => {
    // Define tiers inside the useMemo callback
    const tiers = [
      { id: '1', name: 'Bronze' }, 
      { id: '2', name: 'Silver' },
      { id: '3', name: 'Gold' }, 
  ...
  // Define interface for condition config
  interface ConditionConfig {
    id: string;
    label: string;
    options: Array<{id: string; name: string; icon?: string; [key: string]: any}>;
    state: FilterState;
  }

  // Prepare conditions for ConditionsPanel
  const conditionConfigs = useMemo(() => {
    // Don't reference the outer scope options variable directly
    const configs: ConditionConfig[] = [];
    
    if (!data?.compositions) return configs;
    
    // Process traits with tier info directly in this useMemo for conditioning
    ...
    configs.push({
      id: 'trait',
      label: 'Traits',
      options: condOptions.trait,
      state: conds.trait
    });
    
    return configs;
  }, [data, condSearch, conds]);
